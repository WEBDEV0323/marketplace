...
<main class="my-account-page">
  ...
  @endif
            ...
            @if(Session::has('message_success_verified'))
                <p class="loginsuccess text-success"> {{session('message_success_verified')}} </p>
            @endif
            @if(Session::has('message_error_verified') && Session::has('mail_resend_get'))
                <form method="POST" action="{{ route('resend_verification.email') }}">
                    @csrf                             
                        <div class="form-group position-relative">
                            <input type="hidden" class="form-control" id="email" placeholder="Enter Email"  name="email"  value="{{session('mail_resend_get')}}" required  >
                        </div>                                      
                    <p class="loginsuccess text-danger"> {{session('message_error_verified')}} <button type="submit" class="resend_login_here"> here.</button> </p>
                </form>
                {{-- <p class="loginsuccess text-danger"> {{session('message_error_verified')}}<a  href="user-resend-verification-email"> here.</a> </p> --}}
            @endif

            @if(Session::has('status_already'))
                <p class="loginsuccess text-success"> {{session('status_already')}}</p>
            @endif
            @if(Session::has('status_email_error'))
                <p class="loginsuccess text-danger"> {{session('status_email_error')}}</p>
            @endif
