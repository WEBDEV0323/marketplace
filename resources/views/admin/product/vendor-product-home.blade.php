                        ...
                        <table class="table table-head-bg table-head-primary table table-striped table-bordered basic-datatable" cellspacing="0" width="100%">
                            ...
                            @if(isset($products))
                                ...
                                <tr>
                                    ...
                                    @if($product['flags'] == 1)
                                    <td><span class="badge badge-success badge-sm badge-pill">{{App\Models\Product::STATUS_ACTIVE}}</span></td>
                                    @else
                                    <td><span class="badge badge-danger badge-sm badge-pill">{{App\Models\Product::STATUS_NOT_ACTIVE}}</span></td>
                                    @endif
                                    <td> @if(Auth::guard('admin')->check()) <a href = '{{url("dashboard/product/$product[id]")}}' class="btn btn-primary btn-sm m-1">Edit</a> <a href = "{{ route('product.delete', ['id' => $product['id']]) }}" class="btn btn-danger btn-sm m-1">Delete </a>@endif
                                    <a href = '{{url("vendor/qauntity-update/$product[id]")}}' class="btn btn-primary btn-sm m-1">Stock Update</a></td>
                                </tr>
                                @endforeach
                            @endif  
